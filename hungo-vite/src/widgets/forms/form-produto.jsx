...
const ProdutoForm = () => {
    ...
    // Carregar categorias e insumos ao inicializar
    useEffect(() => {
        const fetchCategorias = async () => {
            try {
                const response = await api.get('/categoria');
        ...
        // Fetch do Produto para Edição
        const fetchProduto = async () => {
            if (id) {
                try {
                    const response = await api.get(`/produtos/${id}`); // Alterado para /produtos/{id}
                    const produto = response.data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (id) {
                // Atualizar produto existente
                await api.put(`/produtos/${id}`, produtoDTO); // Alterado para /produtos/{id}
                setAlertMessage('Produto atualizado com sucesso!');
            } else {
                // Criar novo produto
                await api.post('/produtos', produtoDTO); // Alterado para /produtos
                setAlertMessage('Produto cadastrado com sucesso!');
            }
