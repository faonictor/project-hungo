...
import { ptBR } from "date-fns/locale";  // Para garantir que a data seja formatada em português
import {
    PlusSmallIcon,
    PlusCircleIcon,
    SquaresPlusIcon,
...
const VendaForm = () => {
    ...
    const handleEndVenda = async () => {
        try {
            ...
            // Envia o PUT para atualizar a data de encerramento
            await api.put(`/venda/${vendaToEnd}`, { dataFimVenda: dataAtual });

            // Recarrega as vendas após o encerramento
            fetchVendas();

            // Se a venda selecionada foi encerrada, limpa a seleção
            ...
            // Restaura o estado de carregamento e fecha o modal
            setLoading(false);
    ...
    const handleDeletePedido = async (pedidoId) => {
        try {
            ...
            setAlertMessage("Erro ao excluir pedido");
            setAlertColor("red");
        } finally {
            setLoading(false);
        }
                                    ...
                                    {pedidos.length > 0 ? (
                                        pedidos.map((pedido) => (
                                            <tr key={pedido.id}>
                                                ...
                                                {/*<td className="px-4 py-2 border-b">*/}
                                                {/*    {pedido.dataHora*/}
                                                {/*        ? format(new Date(pedido.dataHora), "dd 'de' MMMM 'de' yyyy, HH:mm:ss", {locale: ptBR})*/}
                                                {/*        : "Data inválida"}*/}
                                                {/*</td>*/}
                                                <td className="px-2 py-2 border-b">
                                                    <Button
