...
const VendaForm = () => {
    ...
    const openDeleteModal = async (vendaId, event) => {
        event.stopPropagation();

        try {
            const response = await api.get(`/pedido/venda/${vendaId}`);
            const pedidosVenda = response.data;
            ...
            // Se não houver pedidos, permite abrir o modal de confirmação
            setVendaToDelete(vendaId);
            setShowDeleteModal(true);
        } catch (error) {
            setAlertMessage("Erro ao verificar pedidos da venda.");
            setAlertColor("red");
        }
    };

    const openEndModal = async (vendaId, event) => {
        event.stopPropagation();
        ...
        try {
            ...
            const pedidosNaoPagos = pedidosVenda.filter(pedido => pedido.statusPedido !== "Pago");
